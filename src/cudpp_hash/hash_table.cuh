...
//}
inline __device__ __host__ Entry make_entry(unsigned long long key, unsigned short value) {
	return (Entry(key) << 16) + value;
}
...
inline __device__ __host__ unsigned long long get_key(Entry entry) {
	return (unsigned long long)(entry >> 16);
}
...
//}
inline __device__ __host__ unsigned short get_value(Entry entry) {
	return (unsigned short)(entry & 0xffff);
}
...
template <unsigned kNumHashFunctions> __device__
unsigned short retrieve(const unsigned long long                     query_key,
	const unsigned                      table_size,
	const Entry                        *table,
	const Functions<kNumHashFunctions>  constants,
	const uint2                         stash_constants,
...
#pragma unroll
	for (unsigned i = 1; i < kNumHashFunctions; ++i) {
		if (key != query_key && key != kKeyEmpty) {
			num_probes++;
			entry = table[locations[i]];
			key = get_key(entry);
		}
